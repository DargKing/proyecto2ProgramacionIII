...
import vistas.componentes.HeaderEgresos;
 ...
 * @author yorman
 */
public class MostrarEgresos extends javax.swing.JPanel {

    BaseDeDatos database;
    int tipo;

    /**
     * Creates new form MostrarEgresos
     */
    public MostrarEgresos(VentanaPrincipal ventana, int tipo) {
        initComponents();
        ...
        ArrayList<CompraMaterialMedico> compras = this.database.getComprasMaterialMedico();
        
        this.container.add(new HeaderEgresos());
        
        if (tipo == 0) {
            this.semana.setVisible(false);
            this.tituloSemana.setVisible(false);
            this.ingresos.setText("Egresos (Dia)");

            for (CompraMaterialMedico compra : compras) {
                if (Integer.parseInt(compra.getFechaDeCompra().split("-")[2]) == dia
                        && Integer.parseInt(compra.getFechaDeCompra().split("-")[1]) == mes
                        && Integer.parseInt(compra.getFechaDeCompra().split("-")[0]) == año) {
                    HeaderEgresos egreso = new HeaderEgresos(compra);

                    this.container.add(egreso);
                }
            }
        }
        ...
        if (tipo == 1) {
            ...
            this.ingresos.setText("Egresos (Semana)");

            for (CompraMaterialMedico compra : compras) {
                if ((Integer.parseInt(compra.getFechaDeCompra().split("-")[2]) / 7 + 1) == semana
                        && Integer.parseInt(compra.getFechaDeCompra().split("-")[1]) == mes
                        && Integer.parseInt(compra.getFechaDeCompra().split("-")[0]) == año) {
                    HeaderEgresos egreso = new HeaderEgresos(compra);
        ...
        if (tipo == 2) {
            ...
            this.semana.setVisible(false);
            this.tituloSemana.setVisible(false);
            this.ingresos.setText("Egresos (Mes)");

            for (CompraMaterialMedico compra : compras) {
                if (Integer.parseInt(compra.getFechaDeCompra().split("-")[1]) == mes
                        && Integer.parseInt(compra.getFechaDeCompra().split("-")[0]) == año) {
                    HeaderEgresos egreso = new HeaderEgresos(compra);
